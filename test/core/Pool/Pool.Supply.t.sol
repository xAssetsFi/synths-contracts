...
contract PoolSupplyTest is PoolSetup {
    function testFuzz_supply_inEmptyPosition(
        uint256 amount
    ) public assumeValidSupplyAmount(amount, wxfi) {
    ...
    function testFuzz_supply_inNonEmptyPosition_SameAsset(
        uint256 amount
    ) public assumeValidSupplyAmount(amount, wxfi) {
    ...
    function testFuzz_supply_inNonEmptyPosition_DifferentAsset(
        uint256 amount
    )
        public
    ...
    function testFuzz_supplyAndBorrow(
        uint256 amount
    ) public assumeValidSupplyAmount(amount, wxfi) {
        uint256 borrowAmount = (amount * WAD) /
    ...
    function testFuzz_supplyETHAndBorrow(uint256 collateral) public {
        vm.assume(collateral > 4); // later will be divided by 4 due to overcollateral model
        ...
        uint loanFee = 100;
        assertEq(
            xusdAmountAfter - xusdAmountBefore,
            borrowAmount - (borrowAmount * loanFee) / PRECISION
        );
