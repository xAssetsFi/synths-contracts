...
contract PoolDataProvider is IPoolDataProvider, UUPSProxy {
    ...
    function getAggregatedPoolData(
        address user
    ) external view returns (AggregatedPoolData memory data) {
    ...
    function getUserPoolData(
        address user
    ) public view returns (UserPoolData memory data) {
        IPool pool = provider().pool();
        ...
        data.collateralValue = pool.totalPositionCollateralValue(
            data.position.collaterals
        );
    ...
    function reCalcHf(
        ...
        if (params.collateralAmount > 0) {
            if (index == -1)
                ...
                position.collaterals[uint8(index)].amount += uint256(
                    params.collateralAmount
                );
        } else if (params.collateralAmount < 0)
            position.collaterals[uint8(index)].amount -=
                type(uint256).max -
                uint256(params.collateralAmount) +
                1;

        uint256 shares = pool.debtShares().balanceOf(user);

        if (params.debtAmount > 0)
            shares += pool.convertToShares(uint256(params.debtAmount));
        else if (params.debtAmount < 0)
            shares -= pool.convertToShares(
                type(uint256).max - uint256(params.debtAmount) + 1
            );
    ...
    function maxXUSDBorrow(
        ...
        uint256 collateralValue = pool.totalPositionCollateralValue(
            position.collaterals
        );

        uint256 _totalDebt = pool.convertToAssets(
            pool.debtShares().balanceOf(user)
        );

        uint256 maxBorrowWithoutDebt = ((
            (((collateralValue * 10 ** provider().xusd().decimals()) *
                PRECISION) / pool.collateralRatio())
        ) / WAD);

        maxBorrow = maxBorrowWithoutDebt > _totalDebt
            ? maxBorrowWithoutDebt - _totalDebt
            : 0;
    }
        ...
        returns (uint256 tokenAmount, uint256 dollarAmountInTokenDecimals)
    {
        ...
        uint256 _totalCollateralValue = pool.totalPositionCollateralValue(
            position.collaterals
        );

        uint256 _totalXUSDDebt = pool.convertToAssets(
            pool.debtShares().balanceOf(user)
        );
        ...
        if (
            ...
            (_totalXUSDDebt * _collateralRatio) / PRECISION
        ) return (0, 0);

        uint256 _maxWithdrawAmount = _totalCollateralValue -
            ((_totalXUSDDebt * _collateralRatio) / PRECISION); // _maxAllowedToWithdrawInDollars

        uint256 _collateralValue = pool.calculateCollateralValue(
            position.collaterals[uint8(index)].token,
            position.collaterals[uint8(index)].amount
        );
        ...
        dollarAmountInTokenDecimals = _maxWithdrawAmount < _collateralValue
            ? _maxWithdrawAmount
            : _collateralValue;

        tokenAmount =
            (
                ((dollarAmountInTokenDecimals * oracle.precision()) /
                    _tokenPrice)
            ) /
            tokenDecimalsDelta;
    ...
    function findLiquidationOpportunity(
        address[] calldata users
    ) external view returns (address[] memory token, uint256[] memory shares) {
        IPool pool = provider().pool();
        IOracleAdapter oracle = provider().oracle();
        ...
        for (uint i = 0; i < users.length; i++) {
            ...
            for (uint j = 0; j < position.collaterals.length; j++) {
                IPool.CollateralData memory collateral = position.collaterals[
                    j
                ];
    ...
    function _getMaxSharesToLiquidate(
        ...
        uint256 deductions = (collateral.amount * (bonusPoint + penaltyPoint)) /
            PRECISION;

        uint256 collateralAmountToLiquidate = collateral.amount - deductions;

        collateralAmountToLiquidate =
            (collateralAmountToLiquidate / 1000) *
            1023; // TODO: refactor this

        uint256 xusdAmount = (collateralAmountToLiquidate * collateralPrice) /
            oraclePrecision;

        xusdAmount *=
            xusdPrecision /
            10 ** IERC20Metadata(collateral.token).decimals();
    ...
    function _calculateHealthFactor(
        ...
        uint256 totalUsdCollateralValue = _totalPositionCollateralValue(
            collateralData,
            oracle,
            oraclePrecision
        );
    ...
    ) public view returns (Token[] memory data) {
    ...
    function _copyPositionAndPushCollateral(
        ...
        newPosition.collaterals[newPosition.collaterals.length - 1] = IPool
            .CollateralData(
