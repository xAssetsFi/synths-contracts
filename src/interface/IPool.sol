...
interface IPool {
    ...
    function isPositionExist(
        address user
    ) external view returns (bool isExists);
    ...
    function totalFundsOnPlatforms()
        external
        view
        returns (uint256 totalFundsOnPlatforms);
    ...
    function liquidationBonusPercentagePoint()
        external
        view
        returns (uint32 ratio);
    ...
    /// @dev ratio scaled by PRECISION
    function liquidationPenaltyPercentagePoint()
        external
        view
        returns (uint32 ratio);
