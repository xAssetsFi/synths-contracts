...
contract Exchanger is IExchanger, UUPSProxy {
    ...
    mapping(address user => mapping(address synthOut => Settlement))
    ...
    function _chargeFee(
        ...
        (uint256 _swapFee, uint256 _rewarderFee, uint256 _burned) = _calcFee(
            amountIn
        );
        ...
        if (_rewarderFee > 0) {
            ...
            uint256 amountOut = _chargeFeeInXUSD(
                synthIn,
                _rewarderFee,
                owner,
                address(this)
    ...
    function settle(
        address user,
        address synth,
        address settlementCompensationReceiver
    ) external noPaused onlySynth(synth) {
        ...
        (bool success, ) = payable(settlementCompensationReceiver).call{
            value: settlement.settleReserve
        }("");
    ...
    function totalFunds() public view returns (uint256 tf) {
        ...
        for (uint i = 0; i < _synths.length; i++) {
            ...
            tf +=
                ((synth.totalSupply() * WAD) * oracle.getPrice(_synths[i])) /
    ...
    function _previewSwap(
        ...
        amountOut =
            (amountIn * oracle.getPrice(synthIn)) /
            oracle.getPrice(synthOut);
    }

    function getSettlement(
        address user,
        address synth
    ) external view onlySynth(synth) returns (Settlement memory settlement) {
    ...
    ) external onlyOwner noZeroAddress(_feeReceiver) {
