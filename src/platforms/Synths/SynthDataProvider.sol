...
contract SynthDataProvider is ISynthDataProvider, UUPSProxy {
    ...
    function aggregateSynthData(
        address user
    ) public view returns (AggregateSynthData memory data) {
    ...
    function synthData(
        address synth,
        address user
    ) public view returns (SynthData memory data) {
    ...
    function getUserSynthData(
        address synth,
        address user
    ) public view returns (UserSynthData memory data) {
        data = UserSynthData(
            ISynth(synth).balanceOf(user),
            provider().exchanger().getSettlement(user, synth)
        );
    }
