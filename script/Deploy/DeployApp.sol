...
abstract contract DeployApp is
    Script,
    Fork,
    DeploymentSettings,
    DeployComponents
{
    ...
    function _setupOracleAdapter(DiaOracleAdapter _oracleAdapter) internal {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            _oracleAdapter.setKey(
                address(assets[i].tokenAddress),
                string.concat(assets[i].symbol, "/USD")
            );
        }

        for (uint256 i = 0; i < collaterals.length; i++) {
            _oracleAdapter.setKey(
                address(collaterals[i].tokenAddress),
                string.concat(collaterals[i].symbol, "/USD")
            );
        }
        vm.stopBroadcast();
    }
    ...
    function _createSynths(
        Synth _synthImplementation,
        Provider _provider
    ) internal {
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            Synth synth = _deploySynths(
                _provider,
                _synthImplementation,
                name,
                symbol
            );
