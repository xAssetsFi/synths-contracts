...
abstract contract DeployComponents is Script, Deploy, DeploymentSettings {
    ...
    function _deployExchanger(
        Provider provider
    ) internal BroadcastAndWrite("exchanger") returns (Exchanger exchanger) {
    ...
    function _deployDiaOracle(
        string[] memory keys,
        uint256[] memory prices
    ) internal BroadcastAndWrite("diaOracle") returns (DiaOracle _diaOracle) {
    ...
    function _deployDebtShares(
        Provider provider,
        Synth xusd
    ) internal BroadcastAndWrite("debtShares") returns (DebtShares debtShares) {
        debtShares = _deployDebtShares(
            owner,
            address(provider),
            "xAssets debt shares",
            "xDS"
    ...
    ) internal BroadcastAndWrite("pool") returns (Pool pool) {
        pool = _deployPool(
            owner,
            address(provider),
            wxfi,
            address(debtShares),
            poolSettings.collateralRatio,
            poolSettings.liquidationRatio,
            poolSettings.liquidationPenaltyPercentagePoint,
            poolSettings.liquidationBonusPercentagePoint,
            poolSettings.loanFee,
            poolSettings.cooldownPeriod
        );
    ...
    function _deployDiaOracleAdapter(
        Provider provider,
        address _diaOracle
    )
        internal
    ...
    {
        oracleAdapter = _deployDiaOracleAdapter(
            owner,
            address(provider),
            _diaOracle
        );
    ...
    function _deployPoolDataProvider(
        Provider provider
    )
        internal
    ...
    function _deploySynthDataProvider(
        Provider provider
    )
        internal
    ...
    function _deployXUSD(
        Provider provider
    ) internal BroadcastAndWrite("xusd") returns (Synth xusd) {
